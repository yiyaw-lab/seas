...
def _judge_with_model(job, model):
    """One model call returning (judgment_dict_or_None, latency_s, out_len)."""
    t0 = time.time()
    try:
        reply = observe.generate_observations(f"{sf.SYSTEM}\n\n{job}", model)
    except Exception as exc:
        return {"_error": f"{type(exc).__name__}: {exc}"}, time.time() - t0, 0
    return sf._extract_json(reply), time.time() - t0, len(reply or "")
...
def run(models, n):
    ...
    per_model = {m: {"rows": [], "latency": [], "out_len": []} for m in models}
    ...
    for i, sig in enumerate(signals, 1):
        ...
        for m in models:
            judgment, lat, olen = _judge_with_model(job, m)
            ...
            print(f"    {m:22s} -> {row.get('outcome'):12s} "
                  f"({lat:.1f}s)")

    # Summary table.
    print("\n" + "=" * 72)
    print(f"{'model':24s} {'find':>5s} {'over':>5s} {'probe':>6s} "
          f"{'pass%':>6s} {'src':>5s} {'qv%':>5s} {'s/call':>7s}")
    print("-" * 72)
    summary = {}
    for m in models:
        ...
        sc = f"{agg['avg_sources_cited']:.1f}" if agg["avg_sources_cited"] is not None else "-"
        ...
        print(f"{m:24s} {agg['findings']:>5d} {agg['overclaims']:>5d} "
              f"{agg['probes']:>6d} {pr:>6s} {sc:>5s} {qv:>5s} {sl:>7s}")
    print("=" * 72)
    print("find=clean findings  over=claimed-but-failed-gate  pass%=findings/claims")
    print("src=avg sources cited  qv%=quotes verifiable in source  s/call=latency\n")
